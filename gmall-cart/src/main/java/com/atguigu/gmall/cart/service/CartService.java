    ...
    public List<Cart> queryCarts() {
        ...
        if (!CollectionUtils.isEmpty(cartJsonList)) {
            unLoginCarts = cartJsonList.stream().map(cartJson -> {
                ...
                String price = this.stringRedisTemplate.opsForValue().get(PRICE_PREFIX + cart.getSkuId());
                cart.setCurrentPrice(BigDecimal.valueOf(Long.parseLong(price)));
                return cart;
...
//            cart.setCurrentPrice(BigDecimal.valueOf(Long.parseLong(price)));
    ...
    public void updateCart(Cart cart) {
        ...
        //获取购物车
        BoundHashOperations<String, Object, Object> boundHashOps = this.stringRedisTemplate.boundHashOps(key);
        Integer count = cart.getCount();
        //判断更新的这条记录，在购物车中有没有
